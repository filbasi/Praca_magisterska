function [Z] = fun2(Z,Y,nx,ny,nz,pfun2)

    Zz(:,:,:)=Z(:,:,:); % Save primary state of matrix 'Z' 
    if pfun2==true
    Z(:,:,nz)=((Y(:,:,nz-1)==1) & (Y (:,:,nz)==1)); % Non-isolated clusters between last and next-to-last images
    end;
    Z(:,:,nz)=((Z(:,:,nz)==1)   | (Zz(:,:,nz)==1)); 
%% Main Loop
    ...
    while true
        ...
        for k=1:length(rows)
              ...
              Z(rows(k),cols(k),i-1)=1;
                ...
                if (cols(k)~=ny) && A(rows(k),cols(k))~=2 && (Y(rows(k),cols(k)+1,i-1)==1) 
                    Z(rows(k),cols(k)+1,i-1)=1;
                    A(rows(k),cols(k)+1)=1;
                end
